...
def padronizar_data_quatro_digitos(data_str):
    """
    Padroniza '25' para '2025' para garantir o merge das datas.
    """
...
def converter_br_para_float(valor):
    """
    Conversão simples e pura para float, sem dividir nada aqui.
    A lógica de correção fica APENAS na unificação.
    """
    if pd.isna(valor) or valor == "": return 0.0
    if isinstance(valor, (int, float)): return float(valor)
    
    valor_str = str(valor).strip().replace('\xa0', '').replace('R$', '').strip()
            ...
            for linha in linhas:
                ...
                if tecnico_atual_aprov:
                    ...
                    if re.match(r"\d{2}/\d{2}/\d{2}", txt_cel0):
                        try:
                            if len(celulas) >= 4:
                                dados.append([txt_cel0.split()[0], arquivo.name, tecnico_atual_aprov, celulas[1].get_text(strip=True), celulas[2].get_text(strip=True), celulas[3].get_text(strip=True)])
                        except: continue
        except Exception as e: st.error(f"Erro no arquivo {arquivo.name}: {e}")
    return dados
...
def atualizar_planilha_preservando_formato(sh, nome_aba, df_final):
    ...
    if dados_para_enviar: ws.update('A2', dados_para_enviar)
    return True
...
def processar_unificacao():
    try:
        ...
        # Padroniza Datas (25 -> 2025)
        if 'Data' in df_com.columns: df_com['Data'] = df_com['Data'].apply(padronizar_data_quatro_digitos)
        if 'Data' in df_aprov.columns: df_aprov['Data'] = df_aprov['Data'].apply(padronizar_data_quatro_digitos)

        # Conversão Básica (String -> Float)
        ...
        df_final = pd.merge(df_com, df_aprov, left_on=['Key_D', 'Key_T'], right_on=['Key_D', 'Key_T'], how='outer', suffixes=('_C', '_A'))
...
if senha == verificar_acesso():
    ...
    with aba1:
        ...
        if files_com:
            ...
            if dados_c: df_comissao_global = pd.DataFrame(dados_c, columns=["Data Processamento", "Nome do Arquivo", "Sigla Técnico", "Horas Vendidas"])
    ...
    with aba2:
        ...
        if files_aprov:
            ...
            if dados_a: df_aprov_global = pd.DataFrame(dados_a, columns=["Data", "Arquivo", "Técnico", "Disp", "TP", "TG"])
